...
| server.subKeepAlivePeriodInSeconds ** (in seconds) | **0**             | The interval at which the connector checks whether OPC-UA subscriptions are still “alive” (i.e., the publish/data-change flow is not stalled). If set to `0`, this check is disabled by default. If a subscription shows no activity longer than this threshold, the connector treats it as expired and triggers resubscription to restore updates. |
