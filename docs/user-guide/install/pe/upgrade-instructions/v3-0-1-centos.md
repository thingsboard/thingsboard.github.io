...
**NOTE**: These upgrade steps are applicable for ThingsBoard version 3.0PE. In order to upgrade to 3.0.1PE you need to [**upgrade to 3.0PE first**](/docs/user-guide/install/pe/upgrade-instructions/v3.0-centos/).
...
```bash
{% include_relative resources/3.0.1pe/thingsboard-centos-download.sh %}
```
{: .copy-code}  
...
```bash
{% include_relative resources/3.0.1pe/thingsboard-centos-installation.sh %}
```
{: .copy-code}  
