...
**NOTE**: These upgrade steps are applicable for ThingsBoard version 3.2.1PE. In order to upgrade to 3.2.2PE you need to [**upgrade to 3.2.1PE first**](/docs/user-guide/install/pe/upgrade-instructions/v3.2.1-centos/).
...
```bash
{% include_relative resources/3.2.2pe/thingsboard-centos-download.sh %}
```
{: .copy-code}  
...
```bash
{% include_relative resources/3.2.2pe/thingsboard-centos-installation.sh %}
```
{: .copy-code}  
