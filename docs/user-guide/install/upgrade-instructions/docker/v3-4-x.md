---
layout: docwithnav
title: ThingsBoard CE v3.4.x upgrade instructions for Docker
description: ThingsBoard CE v3.4.x upgrade guide for Docker
active-menu-item-click: "true"
breadcrumbs: true
breadcrumbs-steps: 2
breadcrumbs-show: 2

---

* TOC
{:toc}

### Upgrading ThingsBoard CE to 3.4.4

{% capture difference %}
**NOTE:**
<br>
These upgrade steps are applicable for ThingsBoard version 3.4.3. In order to upgrade to 3.4.4 you need to [**upgrade to 3.4.3 first**](#upgrading-thingsboard-ce-to-343).
{% endcapture %}
{% include templates/info-banner.md content=difference %}

#### ThingsBoard image download

Download ThingsBoard Docker image:

```bash
docker pull thingsboard/tb-postgres:3.4.4
```
{: .copy-code}

#### ThingsBoard service upgrade

* Stop ThingsBoard `mytb` container if it is running.

```bash
docker compose stop mytb
```
 {: .copy-code}

{% capture dockerComposeStandalone %}
If you still rely on Docker Compose as docker-compose (with a hyphen) execute next command:
<br>**docker-compose stop mytb**
{% endcapture %}
{% include templates/info-banner.md content=dockerComposeStandalone %}

* Update docker-compose.yml

```yml
< . . . >
  mytbce:
    image: "thingsboard/tb-postgres:3.4.4"
< . . . >
```
{: .copy-code}

* Start ThingsBoard:

```bash
docker compose up -d
```
{: .copy-code}

{% capture dockerComposeStandalone %}
If you still rely on Docker Compose as docker-compose (with a hyphen) execute next command:
<br>**docker-compose up -d**
{% endcapture %}
{% include templates/info-banner.md content=dockerComposeStandalone %}

### Upgrading ThingsBoard CE to 3.4.3

{% capture difference %}
**NOTE:**
<br>
These upgrade steps are applicable for ThingsBoard version 3.4.2. In order to upgrade to 3.4.3 you need to [**upgrade to 3.4.2 first**](#upgrading-thingsboard-ce-to-342).
{% endcapture %}
{% include templates/info-banner.md content=difference %}

#### ThingsBoard image download

Download ThingsBoard Docker image:

```bash
docker pull thingsboard/tb-postgres:3.4.3
```
{: .copy-code}

#### ThingsBoard service upgrade

* Stop ThingsBoard `mytb` container if it is running.

```bash
docker compose stop mytb
```
 {: .copy-code}

{% capture dockerComposeStandalone %}
If you still rely on Docker Compose as docker-compose (with a hyphen) execute next command:
<br>**docker-compose stop mytb**
{% endcapture %}
{% include templates/info-banner.md content=dockerComposeStandalone %}

* Update docker-compose.yml

```yml
< . . . >
  mytbce:
    image: "thingsboard/tb-postgres:3.4.3"
< . . . >
```
{: .copy-code}

* Start ThingsBoard:

```bash
docker compose up -d
```
{: .copy-code}

{% capture dockerComposeStandalone %}
If you still rely on Docker Compose as docker-compose (with a hyphen) execute next command:
<br>**docker-compose up -d**
{% endcapture %}
{% include templates/info-banner.md content=dockerComposeStandalone %}

### Upgrading ThingsBoard CE to 3.4.2

{% capture difference %}
**NOTE:**
<br>
These upgrade steps are applicable for ThingsBoard version 3.4.1. In order to upgrade to 3.4.2 you need to [**upgrade to 3.4.1 first**](#upgrading-thingsboard-ce-to-341).
{% endcapture %}
{% include templates/info-banner.md content=difference %}

#### ThingsBoard image download

Download ThingsBoard Docker image:

```bash
docker pull thingsboard/tb-postgres:3.4.2
```
{: .copy-code}

#### ThingsBoard service upgrade

* Stop ThingsBoard `mytb` container if it is running.

```bash
docker compose stop mytb
```
 {: .copy-code}

{% capture dockerComposeStandalone %}
If you still rely on Docker Compose as docker-compose (with a hyphen) execute next command:
<br>**docker-compose stop mytb**
{% endcapture %}
{% include templates/info-banner.md content=dockerComposeStandalone %}

* Update docker-compose.yml

```yml
< . . . >
  mytbce:
    image: "thingsboard/tb-postgres:3.4.2"
< . . . >
```
{: .copy-code}

* Change `upgradeversion` variable:

```bash
echo '3.4.1' | sudo tee ~/.mytb-data/.upgradeversion
```
{: .copy-code}

* Execute the following commands to upgrade ThingsBoard to the new version:

```bash
docker run -it -v ~/.mytb-data:/data --rm thingsboard/tb-postgres:3.4.2 upgrade-tb.sh
```
{: .copy-code}

* Start ThingsBoard:

```bash
docker compose up -d
```
{: .copy-code}

{% capture dockerComposeStandalone %}
If you still rely on Docker Compose as docker-compose (with a hyphen) execute next command:
<br>**docker-compose up -d**
{% endcapture %}
{% include templates/info-banner.md content=dockerComposeStandalone %}

{% capture default-jwt %}
Update the JWT signing key if you use the default one "thingsboardDefaultSigningKey" on production environments. See [JWT security settings](/docs/ce/user-guide/ui/security-settings/#jwt-security-settings) for details.
{% endcapture %}
{% include templates/info-banner.md content=default-jwt %}

### Upgrading ThingsBoard CE to 3.4.1

{% capture difference %}
**NOTE:**
<br>
These upgrade steps are applicable for ThingsBoard version 3.4. In order to upgrade to 3.4.1 you need to [**upgrade to 3.4 first**](#upgrading-thingsboard-ce-to-34).
{% endcapture %}
{% include templates/info-banner.md content=difference %}

#### ThingsBoard image download

Download ThingsBoard Docker image:

```bash
docker pull thingsboard/tb-postgres:3.4.1
```
{: .copy-code}

#### ThingsBoard service upgrade

* Stop ThingsBoard `mytb` container if it is running.

```bash
docker compose stop mytb
```
 {: .copy-code}

{% capture dockerComposeStandalone %}
If you still rely on Docker Compose as docker-compose (with a hyphen) execute next command:
<br>**docker-compose stop mytb**
{% endcapture %}
{% include templates/info-banner.md content=dockerComposeStandalone %}

* Update docker-compose.yml

```yml
< . . . >
  mytbce:
    image: "thingsboard/tb-postgres:3.4.1"
< . . . >
```
{: .copy-code}

* Change `upgradeversion` variable:

```bash
echo '3.4.0' | sudo tee ~/.mytb-data/.upgradeversion
```
{: .copy-code}

* Execute the following commands to upgrade ThingsBoard to the new version:

```bash
docker run -it -v ~/.mytb-data:/data --rm thingsboard/tb-postgres:3.4.1 upgrade-tb.sh
```
{: .copy-code}

* Start ThingsBoard:

```bash
docker compose up -d
```
{: .copy-code}

{% capture dockerComposeStandalone %}
If you still rely on Docker Compose as docker-compose (with a hyphen) execute next command:
<br>**docker-compose up -d**
{% endcapture %}
{% include templates/info-banner.md content=dockerComposeStandalone %}

### Upgrading ThingsBoard CE to 3.4

{% capture difference %}
**NOTE:**
<br>
These upgrade steps are applicable for ThingsBoard version 3.3.4.1. In order to upgrade to 3.4 you need to [**upgrade to 3.3.4.1 first**](/docs/ce/user-guide/install/upgrade-instructions/docker/v3-3-x/#upgrading-thingsboard-ce-to-3341).
{% endcapture %}
{% include templates/info-banner.md content=difference %}

#### ThingsBoard image download

Download ThingsBoard Docker image:

```bash
docker pull thingsboard/tb-postgres:3.4.0
```
{: .copy-code}

#### ThingsBoard service upgrade

* Stop ThingsBoard `mytb` container if it is running.

```bash
docker compose stop mytb
```
 {: .copy-code}

{% capture dockerComposeStandalone %}
If you still rely on Docker Compose as docker-compose (with a hyphen) execute next command:
<br>**docker-compose stop mytb**
{% endcapture %}
{% include templates/info-banner.md content=dockerComposeStandalone %}

* Update docker-compose.yml

```yml
< . . . >
  mytbce:
    image: "thingsboard/tb-postgres:3.4.0"
< . . . >
```
{: .copy-code}

* Change `upgradeversion` variable:

```bash
echo '3.3.4' | sudo tee ~/.mytb-data/.upgradeversion
```
{: .copy-code}

* Execute the following commands to upgrade ThingsBoard to the new version:

```bash
docker run -it -v ~/.mytb-data:/data --rm thingsboard/tb-postgres:3.4.0 upgrade-tb.sh
```
{: .copy-code}

* Start ThingsBoard:

```bash
docker compose up -d
```
{: .copy-code}

{% capture dockerComposeStandalone %}
If you still rely on Docker Compose as docker-compose (with a hyphen) execute next command:
<br>**docker-compose up -d**
{% endcapture %}
{% include templates/info-banner.md content=dockerComposeStandalone %}