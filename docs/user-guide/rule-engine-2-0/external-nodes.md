...
*AWS Region* must correspond to the one in which the SNS Queue(s) are created. Current list of AWS Regions can be found [here](http://docs.aws.amazon.com/general/latest/gr/rande.html)

*Topic ARN pattern* - Administrator can set direct topic name for message publishing 
or set pattern, that will be resolved to the real ARN Topic name using Message metadata. 
For example, topic name depends on Device Type and there is a Message that contains **deviceType** field in Metadata
...
**Published payload** - Node will publish full Message payload to the SNS. If required, Administrator can configure Chain of Transformation Nodes for sending correct Payload to the SNS.

**Outbound message** from this node will contain responce **messageId** and **requestId**
...
- **Delay** - delay in seconds, for which to delay a specific message
- **Message attributes** - delay in seconds, for which to delay a specific message
- **AWS Access key ID**
- **AWS Secret Access Key**
- **AWS region**
...
*Queue URL pattern* - Administrator can set direct Queue URL for message publishing 
or set pattern, that will be resolved to the real Queue URL using Message metadata. 
For example, Queue URL depends on Device Type and there is a Message that contains **deviceType** field in Metadata
{% highlight javascript %}
{
    deviceType: controller
}
...
If required, Administrator can configure Chain of Transformation Nodes for sending correct Payload to the SQS.

**Published attributes** - Administrator can add attributes for publishing message in SQS. It is a collection of <NAME> - <VALUE> pairs.
Both, NAME and VALUE, could be a static values or patterns that will be resolved using Message Payload.

If **FIFO** queue is selected, then Message ID will be used as **deduplication ID** and Message originator as **group ID**

*Outbound message* from this node will contain responce **messageId**, **requestId**, **messageBodyMd5**, **messageAttributesMd5** 
and **sequenceNumber** in Message metadata. Original Message payload, type and originator will not be changed. 

Here is example of configured Node:

![image](/images/user-guide/rule-engine-2-0/sqs node example.png)
 
<br/>

# Kafka Node
Kafka Node is responsible for sending messages to Kafka brokers. Expects messages with any message type. Will send record via Kafka producer to Kafka server.
...
**Published body** - Node will send full Message payload to the Kafka topic. If required, Administrator can configure Chain of Transformation Nodes for sending correct Payload to the SQS.

*Outbound message* from this node will contain responce **offset**, **partition** and **topic** properties in the Message metadata. 
...
**!!! TODO-RE - describe what is Client ID field**
{: style="color:red" }
...
**PEM certificates** - If this authentication types is selected, Administrator will need to provide:
...
**Published body** - Node will send full Message payload to the MQTT topic. If required, Administrator can configure Chain of Transformation Nodes for sending correct Payload.
...
**!!! TODO-RE - implement reconnection to MQTT broker**
{: style="color:red" }
