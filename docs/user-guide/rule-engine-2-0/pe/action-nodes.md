...
Mode when report configuration is taken from message body usually is used when message is generated by [**Generate Report** Scheduler Event](TBD-2.1).

Generate Report node invokes [Reports Server](TBD-2.1) to generate report file using specified dashboard. 

Resulting report file is stored in DataBase and reference to this file is stored in **attachments** field of output message metadata.
...
- **Use system reports server** - if set, [Reports Server](TBD-2.1) endpoint URL will be taken from system configuration (**thingsboard.yml**).
- **Reports server endpoint URL** - endpoint URL of [Reports Server](TBD-2.1).
...
- When [Reports Server](TBD-2.1) is unavailable at the specified endpoint URL.
- When [Reports Server](TBD-2.1) will fail generate report and return corresponding error message. 
