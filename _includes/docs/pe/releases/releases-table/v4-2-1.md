### ThingsBoard PE v4.2.1.1 (Dec 23, 2025)
...
* Core & Rule Engine
  ...
  * Fixed EntityDataTable for multiple keys with aggregation [Reporting]
  * Fixed unit/decimal formatting for text components [Reporting]
  * Fixed report entity loading for entities table
  * Fixed originator name in alarms view
  * Fixed aggregation output for empty result [Reporting 2.0]
  * Improved scheduler: marked Generate Dashboard Report event as deprecated
  * Improved Aggregate latest node: put zero for COUNT/COUNT_UNIQUE when all children are filtered out
  * Improved Duplicate nodes: ensure message processing context is preserved when duplicating for new originators
  * Fixed CF reprocessing when current owner argument used
  * Fixed CF reprocessing for CFs with TS Rolling and Attributes arguments
  * Fixed OPC-UA integration: fixed a bug with casting number types and displaying errors in events
...
* UI
  ...
  * Improved entity groups: unified button labels for "Make public/private" dialogs
