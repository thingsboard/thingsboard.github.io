...
<table>
	...
	<tbody>
		...
		<tr>
			<td>cache.specs.sslRegexBasedCredentials.maxSize</td>
			<td>CACHE_SPECS_SSL_REGEX_BASED_CREDENTIALS_MAX_SIZE</td>
			<td>1</td>
			<td> Cache max size. 0 means the cache is disabled in case CACHE_TYPE=caffeine.
 Defaults to 1, as the cache is designed to store only a single key at any given time. Increasing the size is unnecessary for this use case</td>
		</tr>
	</tbody>
</table>
