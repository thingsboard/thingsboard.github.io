...
But, there is no need to aggregate JSON data, so we can store all the content as one row instead of separate rows for each configuration item.
...
Learn more about JSON value support feature with the [video](https://www.youtube.com/watch?v=ErV5Krwhak0&ab_channel=ThingsBoard).
